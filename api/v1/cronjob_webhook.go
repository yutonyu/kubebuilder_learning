...
- Mutating admission webhook: Tehse can mutate the object while it is being created or
updated, before it gets stored. It can be used to default feilds in a resource requests, e.g. fields in deployment taht are
not sepcified by the user. It can be used to inject sidecar containers.
- Validating admission webhook: These can validate the object while it is being created or updated, before it gets stored.
...
The default webhookconfiguriaotn manifests created alongside yoru Go webhook impelentiaotn use API version v1.
*/
...
/*
We can validate our CRD beyond what's possible with declarative validation. Generally, declarative validation
should be sufficient, but sometimes more advanced use cases call for complex validation.
For instnace, we will see below that we use this to valiate a well-formed ron schedule without making up a long regular expression
...
func (r *CronJob) validateCronJob() error {
	...
	return apierrors.NewInvalid(
		schema.GroupKind{Group: "batch.tutorial.kubebilder.io", Kind: "CronJob"},
		r.Name, allErrs)
}
...
// We will need to validate hte cron schedule is well-formatted
